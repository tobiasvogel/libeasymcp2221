...
/* Error-returning I2C slave functions return MCP_ERR_OK on success or another
 * mcp_err_t value on error. i2c_slave_is_present() is a boolean-style probe
 * and returns 1 when the device ACKs, otherwise 0.
 */

/* Preferred mcp2221_* names. */
mcp_err_t mcp2221_i2c_slave_init(mcp2221_i2c_slave_t *slave, MCP2221 *mcp, uint8_t addr, int force, uint32_t i2c_speed_hz,
						   int reg_bytes, const char *reg_byteorder);
int mcp2221_i2c_slave_is_present(mcp2221_i2c_slave_t *slave);
mcp_err_t mcp2221_i2c_slave_read_register(mcp2221_i2c_slave_t *slave, uint32_t reg, uint8_t *buffer, size_t length,
									int reg_bytes, const char *reg_byteorder);
mcp_err_t mcp2221_i2c_slave_read(mcp2221_i2c_slave_t *slave, uint8_t *buffer, size_t length);
mcp_err_t mcp2221_i2c_slave_write_register(mcp2221_i2c_slave_t *slave, uint32_t reg, const uint8_t *data, size_t length,
									 int reg_bytes, const char *reg_byteorder);
mcp_err_t mcp2221_i2c_slave_write(mcp2221_i2c_slave_t *slave, const uint8_t *data, size_t length);
