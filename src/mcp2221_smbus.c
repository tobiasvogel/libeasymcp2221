...
mcp2221_error_code_t smbus_init(mcp2221_smbus_t *bus, mcp2221_t *existing_mcp, int device_index, uint16_t vid, uint16_t pid, const char *usbserial,
				   uint32_t i2c_speed_hz) {
	return mcp2221_smbus_init(bus, existing_mcp, device_index, vid, pid, usbserial, i2c_speed_hz);
}

void smbus_close(mcp2221_smbus_t *bus) {
	mcp2221_smbus_close(bus);
}

mcp2221_error_code_t smbus_read_byte(mcp2221_smbus_t *bus, uint8_t addr, uint8_t *value) {
	return mcp2221_smbus_read_byte(bus, addr, value);
}

mcp2221_error_code_t smbus_write_byte(mcp2221_smbus_t *bus, uint8_t addr, uint8_t value) {
	return mcp2221_smbus_write_byte(bus, addr, value);
}

mcp2221_error_code_t smbus_read_byte_data(mcp2221_smbus_t *bus, uint8_t addr, uint8_t reg, uint8_t *value) {
	return mcp2221_smbus_read_byte_data(bus, addr, reg, value);
}

mcp2221_error_code_t smbus_write_byte_data(mcp2221_smbus_t *bus, uint8_t addr, uint8_t reg, uint8_t value) {
	return mcp2221_smbus_write_byte_data(bus, addr, reg, value);
}

mcp2221_error_code_t smbus_read_word_data(mcp2221_smbus_t *bus, uint8_t addr, uint8_t reg, int16_t *value) {
	return mcp2221_smbus_read_word_data(bus, addr, reg, value);
}

mcp2221_error_code_t smbus_write_word_data(mcp2221_smbus_t *bus, uint8_t addr, uint8_t reg, int16_t value) {
	return mcp2221_smbus_write_word_data(bus, addr, reg, value);
}

mcp2221_error_code_t smbus_process_call(mcp2221_smbus_t *bus, uint8_t addr, uint8_t reg, int16_t value, int16_t *response) {
	return mcp2221_smbus_process_call(bus, addr, reg, value, response);
}

mcp2221_error_code_t smbus_read_block_data(mcp2221_smbus_t *bus, uint8_t addr, uint8_t reg, uint8_t *buffer, size_t *length) {
	return mcp2221_smbus_read_block_data(bus, addr, reg, buffer, length);
}

mcp2221_error_code_t smbus_write_block_data(mcp2221_smbus_t *bus, uint8_t addr, uint8_t reg, const uint8_t *data, size_t length) {
	return mcp2221_smbus_write_block_data(bus, addr, reg, data, length);
}

mcp2221_error_code_t smbus_block_process_call(mcp2221_smbus_t *bus, uint8_t addr, uint8_t reg, const uint8_t *data, size_t length,
							 uint8_t *response, size_t *resp_len) {
	return mcp2221_smbus_block_process_call(bus, addr, reg, data, length, response, resp_len);
}

mcp2221_error_code_t smbus_read_i2c_block_data(mcp2221_smbus_t *bus, uint8_t addr, uint8_t reg, uint8_t *buffer, size_t length) {
	return mcp2221_smbus_read_i2c_block_data(bus, addr, reg, buffer, length);
}

mcp2221_error_code_t smbus_write_i2c_block_data(mcp2221_smbus_t *bus, uint8_t addr, uint8_t reg, const uint8_t *data, size_t length) {
