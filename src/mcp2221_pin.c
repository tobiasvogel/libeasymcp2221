...
static mcp2221_error_code_t fill_gp_config_from_function(mcp2221_gpio_pin_t pin, mcp2221_pin_function_t function, int out_value,
...
mcp2221_error_code_t mcp2221_pin_set_functions(mcp2221_t *dev, const mcp2221_pin_functions_t *cfg) {
	...
	for (int i = 0; i < 4; i++) {
		sram.gp[i].value = MCP2221_CONFIG_KEEP;
		sram.gp[i].direction = MCP2221_CONFIG_KEEP;
		sram.gp[i].function = MCP2221_CONFIG_KEEP;
	}

	// No changes for other SRAM fields
	sram.int_cfg.pos_edge = MCP2221_CONFIG_KEEP;
	...
	for (int i = 0; i < 4; i++) {
		...
		int outv = cfg->out[i];
		mcp2221_error_code_t r = fill_gp_config_from_function((mcp2221_gpio_pin_t)i, fn, outv, &sram.gp[i]);
		if (r != MCP2221_ERR_OK)
			return r;
	}
